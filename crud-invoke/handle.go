...
func Handle(ctx context.Context, event Event, store *sqlserver.Store) (any, error) {
...
func CrudByName(s *sqlserver.Store, name string) CrudHandler {
	switch name {
	case "databases":
		return Crud[string, sqlserver.Database]{DataAccess: s.Databases}
	case "logins":
		return Crud[string, sqlserver.Login]{DataAccess: s.Logins}
	case "database_access":
		return Crud[sqlserver.DatabaseAccessKey, sqlserver.DatabaseAccess]{DataAccess: s.DatabaseAccess}
	default:
		return nil
	}
}
